...
public class PilotValidator extends DefaultHandler {
    final String parserClass = "org.apache.xerces.parsers.SAXParser";
    StringBuffer result = new StringBuffer("");

    // The parse method in XMLReader prefers an InputSource {pages 45 and 47}
	...
	if (message.equals(""))
	    ...
	    message = "De val som gjorts följer inte cancerregistrets XMLSchema.\n"
     ...
     * System.out.println("Usage: java validate <XML file>"); return; } try {
     * String validationResults = process(args[0]);
     * System.out.println(validationResults); }catch (Exception e) {
     * e.printStackTrace(); } }
     */
	...
	if (result.length() > 800)
	    return;
	result.append("[Warning] " + getLocationString(spex) + ": " + spex.getMessage() + "\n");
    }
	...
	if (result.length() > 800)
	    return;
	result.append("[Error] " + getLocationString(spex) + ": " + spex.getMessage() + "\n");
    }
	...
	if (result.length() > 800)
	    return;
	result.append("[Fatal Error] " + getLocationString(spex) + ": " + spex.getMessage() + "\n");
    }
